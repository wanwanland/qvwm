...
#include "dialog/radio_button.h"
#include "dialog/radio_set.h"
...
void SystemDialog::Exposure(Window win)
{
  ...
  // XXX
  if (XFindContext(display, win, event.m_context, (caddr_t*)&sb) == XCSUCCESS)
    sb->paint();
...
ResourceId SystemDialog::Button1Press(Window win)
{
  ...
  // XXX
  if (XFindContext(display, win, event.m_context, (caddr_t*)&sb)
      == XCSUCCESS) {
    if (sb->GetParent() == frame) {
      // XXX
      XButtonEvent xev;
      xev.button = Button1;
      sb->onButtonPress(xev);
