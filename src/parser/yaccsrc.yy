...
%token <str> VARIABLE MENU SHORTCUT APP KEY IND EXITDLG STARTUP ACC TBSC
%token <str> VAR STRING FUNC PLUS MINUS

%type <str> session, sessions, ind, inds, cmd, cmds, acc, accs
%type <mItem> item, items, sc, scs, dlgitem, dlgitems, tbsc, tbscs
%type <aStream> stream
%type <modifier> mod
%type <sck> keys, key
...
tbscs:	  tbsc tbscs			{ $$ = ChainMenuItem($1, $2); }
	| tbsc				{ $$ = ChainMenuItem($1, NULL); }
	;

tbsc:	  STRING STRING STRING
			{ $$ = MakeExecItem($1, $2, $3); }
	| STRING STRING FUNC
			{ $$ = MakeFuncItem($1, $2, $3); }
	;
